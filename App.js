			...
			<ul>
				{
					spacePhenomena.map(spacePhenomena => {
						...
						const observationStatuses = observationStatuses[index];

						return (
							<li key={spacePhenomena.id}>
									{spacePhenomena.emoji} {spacePhenomena.name} - {observationStatuses}
									{index === 2 && <span> 🔧 (Prepare yourself with top-notch gear!) </span>}
							</li>
						);
					})
				}
